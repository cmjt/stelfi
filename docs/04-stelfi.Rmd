...
domain.sf <- sf::st_as_sf(domain)
smesh <- INLA::inla.mesh.2d(boundary = INLA::inla.sp2segment(domain), 
                            max.edge = 0.75, cutoff = 0.3)
param <- list(mu = 1, alpha = 3, beta = 6, xsigma = 0.2,
             ysigma = 0.2, rho = 0)
fit <- fit_stelfi(times = times, locs = locs, sf = domain.sf, smesh = smesh, parameters = param)
get_coefs(fit)
```
...
fit <- fit_stelfi(times = times, locs = locs, sf = domain.sf, smesh = smesh, parameters = param, GMRF = TRUE)
get_coefs(fit)
res = TMB::sdreport(fit)
show_field(res$par.random, smesh = smesh, sf = domain.sf)
```