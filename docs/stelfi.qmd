...
ggplot(data.frame(x = sim$locs[,1], y = sim$locs[,2], times = sim$times)) +
  geom_point(aes(x = x, y = y, group = seq_along(times))) +
	geom_sf(data = domain, fill = NA, inherit.aes = FALSE) +
	theme_void() + transition_reveal(times)
```

```{r}
fit <- fit_stelfi(times = sim$times, locs = data.frame(x = sim$locs[,1], y = sim$locs[,2]), sf = domain, smesh = smesh, parameters = param)
get_coefs(fit)
...
The estimated GMRF can be plotted using the `show_field()` function once the values have been extracted using `get_fields()` (secc @lgcp).

```{r}
#| message: false
get_fields(fit, smesh) |>
    show_field(smesh = smesh, sf = domain, clip = TRUE) + ggplot2::theme_classic()
```