...
```{r plot dual mesh, echo = FALSE, fig.width=10,fig.height=10,fig.cap="Delauney triangulation of the domain (white) overlain on the Voronoi diagram representing the weights (area surrounding) of each mesh node (diamonds). Observations are plotted as circles, mesh nodes outwith the domain are shown in grey."}
...
```{r plot weights,echo = FALSE, fig.width=10,fig.height=10,fig.cap="Voronoi diagram of the weights (areas in km2 around each mesh node)."}
...
pal <- scales::col_numeric("Blues",
                           exp(as.numeric(pp.res$summary.fixed[1]))*weights[ins])
cols <- pal(exp(as.numeric(pp.res$summary.fixed[1]))*weights[ins])
plot(dual_mesh[ins], col = cols)
plot(nz, add = TRUE)
...
```{r st model,eval = FALSE, echo = FALSE}
## space time SPDE
